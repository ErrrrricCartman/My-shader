    ...
    SubShader
    {
        Pass{
			...
			fixed4 frag(v2f i):SV_TARGET{
				fixed3 tangentLighrDir = normalize(i.lightDir);
				...
				fixed3 diffuse = _LightColor0.rgb * albedo * saturate(dot(tangentLighrDir, tangentNormal));

				fixed3 halfDir = normalize(tangentLighrDir + tangentViewDir);
